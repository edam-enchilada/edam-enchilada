 ...
 * License.
 *
 * The Original Code is EDAM Enchilada's ATOFMSAutoCalibrator class.
 *
 ...
 *
 */
public class ATOFMSAutoCalibrator {
											   ...
											   int spectrumType)
	{
		...
		if (spectrumType == POS)
				if (mass >= 0) {
					return ((Math.sqrt(Math.abs(mass)) - ATOFMSParticles.currCalInfo.posSlope) / 
							ATOFMSParticles.currCalInfo.posIntercept);
				}
				else 
					return ((-2 * (ATOFMSParticles.currCalInfo.posSlope / 
							ATOFMSParticles.currCalInfo.posIntercept)) - 
							((Math.abs(mass) * Math.abs(mass)) - ATOFMSParticles.currCalInfo.posSlope) / 
							ATOFMSParticles.currCalInfo.posIntercept);

		else
				if (mass >= 0) {
					
					return ((Math.sqrt(Math.abs(mass)) - ATOFMSParticles.currCalInfo.negSlope) / 
							ATOFMSParticles.currCalInfo.negIntercept);
				}
				else {
					return ((-2 * (ATOFMSParticles.currCalInfo.negSlope / 
							ATOFMSParticles.currCalInfo.negIntercept)) - 
							(Math.sqrt(Math.abs(mass)) - ATOFMSParticles.currCalInfo.negSlope) / 
							ATOFMSParticles.currCalInfo.negIntercept);
				}
	}
			...
			//int& TOF2, double& MZ2)
	{
		...
		//System.out.println(ATOFMSParticles.currCalInfo.posSlope);
		//System.out.println(ATOFMSParticles.currCalInfo.negSlope);
	...
	private static int MZ2TOF(double mass, int charge)
	{
		if (charge == POS)
		{
			return (int) (((Math.sqrt(Math.abs(mass)) - 
					ATOFMSParticles.currCalInfo.posIntercept) / 
					ATOFMSParticles.currCalInfo.posSlope) 
					/*+ 0.5*/);
		}
		else if (charge == NEG)
		{
			return (int)(((Math.sqrt(Math.abs(mass)) - 
					ATOFMSParticles.currCalInfo.negIntercept) / 
					ATOFMSParticles.currCalInfo.negSlope) 
					/*+ 0.5*/);
		}
		else
			throw new IllegalArgumentException(
					"Must input charge as either ATOFMSAutoCalibrator.POS " +
					"or ATOFMSAutoCalibrator.NEG");
	}

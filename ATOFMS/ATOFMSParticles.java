 ...
 * License.
 *
 * The Original Code is EDAM Enchilada's ATOFMSParticles class.
 *
...
public class ATOFMSParticles {
	...
	protected ArrayList<ATOFMSPeak> peakList = null;
	public int atomID;

	public static ATOFMSPeakParams currPeakParams;
	public static ATOFMSCalInfo currCalInfo;
	
	private double autoPosSlope, autoNegSlope, autoPosIntercept,
		autoNegIntercept;

	public ATOFMSParticles()
	{
		super();
	 ...
	 */
	public ATOFMSParticles(String fname, 
						  String timestr, 
						  float lasPow,
						  float dRate,
						  int sDelay,
						  ...
						  )
	{
		...
		if (currCalInfo.autocal)
		{
			...
			returnVals = ATOFMSAutoCalibrator.autoCalibrate(digitRate, 
					ATOFMSAutoCalibrator.POS, posSpectrum);
			if (returnVals != null)
			{
			...
			returnVals = ATOFMSAutoCalibrator.autoCalibrate(digitRate, 
					ATOFMSAutoCalibrator.NEG, negSpectrum);
			if (returnVals != null)
			{
	 ...
	 */
	public ArrayList<ATOFMSPeak> getPeakList()
	{
		if (peakList!= null)
			return peakList;
		peakList = new ArrayList<ATOFMSPeak>(20);
	...
	private boolean getPosPeaks(int baseline)
	{
		...
		while (i < MAX_BIN_NUMBER)
		{
			...
			{ 
				...
				peakList.add(new ATOFMSPeak(peakHeight, peakArea, getPosMZ(centerIndex)));
				foundPeak = false;
			} // if (foundPeak == true)
			else
				i++;
		...
		{
			ATOFMSPeak peak = peakList.get(k);
	...
	private boolean getNegPeaks(int baseline)
	{
		...
		while (i < MAX_BIN_NUMBER)
		{
			...
			if (foundPeak == true)
			{
				...
				peakList.add(new ATOFMSPeak(peakHeight, peakArea, getNegMZ(centerIndex)));
		...
		{
			ATOFMSPeak peak = peakList.get(l);
			peak.relArea = (float)peak.area/totalArea;
			peakList.set(l, peak);
		}
		return true;
